...
// Call method to extract the selected lap (does NOT trigger rebuilds on its own)
    ...
    List<FlSpot> rpmSpots = _convertToSpots(lapData, 'rpm', 'timestamp',
        113500); //the data, the data key, timestamp key, plausability tolerence value
    List<FlSpot> speedSpots =
        _convertToSpots(lapData, 'speed', 'timestamp', 85);                 // something wrong with this
    List<FlSpot> batteryVoltageSpots =
        _convertToSpots(lapData, 'batteryVoltage', 'timestamp', 100);
    ...
    List<FlSpot> airTempSpots = _convertToSpots(lapData, 'airTemperature', 'timestamp', 100);
                      ...
                      child: LineChart(
                        LineChartData(
                            ...
                            extraLinesData: ExtraLinesData(
                              ...
                              horizontalLines: [
                                HorizontalLine(
                                    y: 14.5, color: Colors.red, strokeWidth: .8),
                                HorizontalLine(
                                    y: 12, color: Colors.blue, strokeWidth: .8),
                              ],
                            ),
                            lineTouchData: LineTouchData(
                              handleBuiltInTouches: true,
                        ...
                        LineChartData(
                            lineBarsData: [
                              LineChartBarData(
                                spots: airTempSpots,
                                //spots:batteryVoltageSpots,
                                color: const Color.fromARGB(255, 3, 194, 242),
                                barWidth: 1,
                                dotData: FlDotData(
                                  show: true,
                                  checkToShowDot: (spot, barData) {
                              ...
                              touchTooltipData: LineTouchTooltipData(
                                ...
                                getTooltipItems:
                                  ...
                                  return touchedSpots
                                    ...
                                    return LineTooltipItem(
                                      // 'Battery: ${touchedSpot.y.toString()} Volts',
                                      'Air Temp: ${touchedSpot.y.toString()} c',
                                      const TextStyle(color: Colors.white),
                                    );
                                  }).toList();
                                },
